...
fn query_filter_keyspace_name<R, T>(
    conn: &Arc<Connection>,
    query_str: &str,
    keyspaces_to_fetch: &[String],
) -> impl Stream<Item = Result<<R as DeserializeRow<'static>>::Target, QueryError>>
where
    R: for<'r> DeserializeRow<'r, Target = T>,
    T: 'static,
{
...
async fn query_keyspaces(
    ...
    let rows = query_filter_keyspace_name::<(String, HashMap<String, String>), _>(
        conn,
...
async fn query_user_defined_types(
    ...
    let rows = query_filter_keyspace_name::<UdtRow, _>(
        conn,
...
async fn query_tables(
    ...
    let rows = query_filter_keyspace_name::<(String, String), _>(
        conn,
...
async fn query_views(
    ...
    let rows = query_filter_keyspace_name::<(String, String, String), _>(
        conn,
...
async fn query_tables_schema(
    ...
    let rows = query_filter_keyspace_name::<RowType, _>(conn,
