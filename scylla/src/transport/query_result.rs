...
use scylla_cql::types::deserialize::row::{DeserializeRow, TypedRowIterator};
...
impl QueryResult {
    ...
    pub fn first_row<'s, R: DeserializeRow<'s>>(&'s self) -> Result<R::Target, FirstRowError> {
    ...
    pub fn maybe_first_row<'s, R: DeserializeRow<'s>>(
        &'s self,
    ) -> Result<Option<R::Target>, RowsError> {
        Ok(self.rows::<R>()?.next().transpose()?)
    }

    /// Returns the only received row.\
    /// Fails if the result is anything else than a single row.\
    pub fn single_row<'s, R: DeserializeRow<'s>>(&'s self) -> Result<R::Target, SingleRowError> {
        match self.rows::<R>() {
            Ok(mut rows) => match rows.next() {
                Some(Ok(row)) => {
                    if rows.len() != 0 {
                        return Err(SingleRowError::UnexpectedRowCount(rows.len() + 1));
                    }
                    Ok(row)
                }
