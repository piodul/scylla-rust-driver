...
impl RawIterator {
    ...
    ///
    /// This is cancel-safe.
    pub async fn next<'r>(&'r mut self) -> Option<Result<ColumnIterator<'r>, QueryError>> {
    ...
    fn is_current_page_exhausted(&self) -> bool {
        self.current_page.rows_remining() == 0
    }
}
...
impl<RowT, CommonTarget> Stream for TypedRowIterator<RowT>
where
    RowT: for<'r> DeserializeRow<'r, Target = CommonTarget>,
    CommonTarget: 'static,
{
    type Item = Result<CommonTarget, QueryError>;
