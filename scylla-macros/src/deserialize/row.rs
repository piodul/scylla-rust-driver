...
struct Field {
    ...
    // If set, uses given type for deserialization. The `impl DeserializeCql`
    // for this type should deserialize to the type specified by this field.
    #[darling(default)]
...
impl DeserializeCommonFieldAttrs for Field {
    ...
    fn deserializer(&self) -> syn::Type {
        self.with.as_ref().unwrap_or(&self.ty).clone()
    }
...
impl Field {
    ...
    fn cql_name_literal(&self) -> syn::LitStr {
        syn::LitStr::new(&self.ident.as_ref().unwrap().to_string(), Span::call_site())
    }
}
...
impl<'sd> TypeCheckAssumeOrderGenerator<'sd> {
    fn generate(&self) -> syn::ImplItemMethod {
        ...
        let field_names = required_fields.iter().map(|f| f.cql_name_literal());
        let field_idents: Vec<_> = (0..required_fields.len())
            .map(|i| quote::format_ident!("f_{}", i))
            .collect();
        let numbers = 0usize..;

        let field_deserializers = required_fields.iter().map(|f| f.deserializer());
            ...
            fn type_check(
                ...
                match specs {
                    [#(#field_idents),*] => {
                        #(
                            // Verify the name
                            if #field_idents.name != #field_names {
...
impl<'sd> DeserializeAssumeOrderGenerator<'sd> {
    fn generate_finalize_field(&self, field: &Field) -> syn::Expr {
        ...
        let crate_path = self.0.struct_attrs().crate_path();
        let deserializer = field.deserializer();
        let constraint_lifetime = self.0.constraint_lifetime();
        parse_quote!(
            {
...
impl<'sd> TypeCheckUnorderedGenerator<'sd> {
    ...
    fn generate(&self) -> syn::ImplItemMethod {
        ...
        let fields = self.0.fields();
        let visited_field_declarations = fields
            .iter()
            .flat_map(|f| Self::generate_visited_flag_decl(f));
